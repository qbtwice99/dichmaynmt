...
test_data = TranslationDataset(path=r'./data/test', exts=('.src', '.tgt'),
                               filter_pred=filter_len, fields=(SRC, TRG))
train_data = TranslationDataset(r'./data/train', exts=('.src', '.tgt'),
                                filter_pred=filter_len, fields=(SRC, TRG))
valid_data = TranslationDataset(r'./data/valid', exts=('.src', '.tgt'),
                                filter_pred=filter_len, fields=(SRC, TRG))
...
def evaluate():
    path = args.model
    # with open path as path2:
    #     path2.seek(0, os.SEEK_END)
    #     while True:
    #         path = path2.readline()
    #         if not lines:
    #             time.sleep(0.1)
